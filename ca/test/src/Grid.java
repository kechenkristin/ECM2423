import java.io.BufferedReader;
import java.io.FileReader;
import java.io.IOException;

public class Grid {
    boolean[][] grid;

    int width;

    int length;


    /**
     * Construct a grid with width and length are known
     */

    /*
    public Grid(int w, int l) {
        this.width = w;
        this.length = l;

        grid = new boolean[width][length];
    }
     */


    public void constructWidthLength(String fileName) {
        BufferedReader reader;


        try {
            reader = new BufferedReader(new FileReader("res/maze/" + fileName));


            String line = reader.readLine();

            /* count how many chars are in a string, remove space */
            int charCount = line.replace(" ", "").length();
            System.out.println(charCount);


            int lineCount = 0;

            while (line != null) {
                System.out.println(line);
                // read next line
                line = reader.readLine();
                lineCount++;
            }

            reader.close();
            System.out.println(lineCount);

            width = charCount;
            length = lineCount;
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    public void constructGrid(String fileName) {
        BufferedReader reader;


        try {
            reader = new BufferedReader(new FileReader("res/maze/" + fileName));

            String line = reader.readLine();


            int lineNo = 0;
            grid = new boolean[length][width];

            while (line != null) {
                String cleanLine = line.replace(" ", "");
                System.out.println(cleanLine);

                for (int i = 0; i < width; i ++) {
                    grid[lineNo][i] = cleanLine.charAt(i) != '#';
                }

                // read next line
                line = reader.readLine();
                lineNo++;
            }

            reader.close();
            System.out.println(lineNo);

            for (int i = 0; i < length; i++) {
                for (int j = 0; j < width; j++) {
                    System.out.println(grid[i][j]);
                }
                System.out.println();
            }

        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    public boolean isPath(int x, int y) {
        return grid[x][y];
    }

    public static void main(String[] args) {
        System.out.println( 12 / 5);
    }


}
